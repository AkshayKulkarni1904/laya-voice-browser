"""Chrome, Edge, Brave, Opera and other Chromium browsers over the Chrome DevTools Protocol on Windows.
...
CHROMIUM_APPS = (
    ChromiumApp(
        ...
        "msedge.exe",
        (
            _PROGRAMFILES_X86 / "Microsoft" / "Edge" / "Application" / "msedge.exe",
    ...
    ChromiumApp(
        ...
        (
            ...
            _PROGRAMFILES_X86 / "Google" / "Chrome" / "Application" / "chrome.exe",
    ...
    ChromiumApp(
        ...
        "launcher.exe",
        (
            _LOCALAPPDATA / "Programs" / "Opera" / "launcher.exe",
    ...
    ChromiumApp(
        ...
        "launcher.exe",
        (
            _LOCALAPPDATA / "Programs" / "Opera GX" / "launcher.exe",
            _LOCALAPPDATA / "Programs" / "Opera GX" / "opera.exe",
        ),
    ),
)
...
def profile_dir(app: ChromiumApp) -> Path:
    return _LOCALAPPDATA / "LayaBrowse" / "profiles" / app.key
...
def app_path(app: ChromiumApp) -> Path | None:
    for candidate in app.default_locations:
        if candidate.is_file():
            return candidate
    reg_path = _query_registry_app_path(app.exe_name)
...
def read_devtools_endpoint(profile: Path) -> str | None:
    ...
    try:
        ...
        if not active_port_file.exists():
            return None
        port, path = active_port_file.read_text(encoding="utf-8").splitlines()[:2]
...
class ChromiumBrowser:
    """Controls the tab you are looking at in LayaBrowse's Chromium window on Windows."""
    ...
    def _connect_or_launch(self) -> CDPConnection:
        ...
        if not path:
            raise RuntimeError(f"{self.app.name} is not installed on this system")
        profile.mkdir(parents=True, exist_ok=True)
        (profile / "DevToolsActivePort").unlink(missing_ok=True)
        
        # On Windows, spawn the browser executable directly with remote debugging flags
        subprocess.Popen(
            [str(path), *launch_arguments(self.app)],
