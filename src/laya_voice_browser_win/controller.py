...
class StreamingController:
    """Serialize every Browser/Laya operation while accepting streaming transcript updates."""
    ...
    def submit(self, event: TranscriptEvent) -> None:
        ...
        if self.island:
            self.island.set_transcript(text)
    ...
    def _on_session_lost(self, exc: Exception) -> None:
        ...
        self.announce(f"browser session ended ({exc})")
    ...
    def _execute(
        ...
        *,
        ...
        self._status("acting", label=action_type)
        if self.island:
            self.island.set_action(f"⚡ {action_type.capitalize()}")
        ...
        self.announce(f"executed: {action['type']} -> {outcome.get('after_url', '')}")
    ...
    def _report_browser_problem(self, exc: Exception) -> None:
        self._status("error", label="Browser Error", ttl=5.0)
    ...
    def _status(self, state: str, **details: Any) -> None:
        if self.island:
            state_map = {
                ...
                "acting": (f"Running {details.get('label', '')}", "executing"),
                "done": ("Success", "executing"),
                "confirm": ("Confirm? (say confirm)", "confirm"),
                "choose": ("Pick a number", "confirm"),
                "unsure": ("Unsure", "thinking"),
                "error": (details.get("label", "Error"), "confirm"),
            }
            lbl, st = state_map.get(state, (state.capitalize(), "listening"))
            self.island.set_status(lbl, st)
